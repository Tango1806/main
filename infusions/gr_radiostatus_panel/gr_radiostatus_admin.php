...
+--------------------------------------------------------+
| DJ Erkennung & PHP 8 Version
| Author: 21Matze
| Webseie fusion-club24.de
+--------------------------------------------------------+
...
if (file_exists(RADIOSTATUS.'locale/'.LOCALESET.'index.php')) {
 ...
 if(version_compare(phpversion(), '8.2', '<')) {
  echo "<div class='admin-message'><font color='red'><strong>Ihre PHP Version ist veraltet bitte nutzen sie aktuelle 8.2 PHP Version</strong> </div></br>";
 }
 //$version_data = dbarray(dbquery("SELECT * FROM ".DB_INFUSIONS." WHERE inf_folder='gr_radiostatus_panel'"));

   $version = dbarray(dbquery("SELECT inf_version FROM ".DB_INFUSIONS." WHERE inf_folder = 'gr_radiostatus_panel'"));
if($locale['version'] > $version['inf_version']){
 // echo "<div class='admin-message'><font color='red'><strong>Upgrade Info hier geht es zur </br><a class='button' href='".RADIOSTATUS."upgrade.php'.$aidlink.''>Install</a></strong> </div></br>";
  echo "<div class='admin-message'><font color='red'><strong>Upgrade Info hier geht es zur <a class='button' href='".RADIOSTATUS."upgrade.php".$aidlink."'>Install</a></strong> </div></br>";
 }



 #$result = dbquery("ALTER TABLE ".DB_GR_RADIOSTATUS." ADD rs_https VARCHAR (255) NOT NULL ");
 #$result = dbquery("ALTER TABLE ".DB_GR_RADIOSTATUS." ADD rs_https_port VARCHAR (255) NOT NULL ");
/*$result = dbquery("ALTER TABLE ".DB_GR_RADIOSTATUS." ADD rs_fm VARCHAR (255) NOT NULL ");
 $result = dbquery("ALTER TABLE ".DB_GR_RADIOSTATUS." ADD rs_radio VARCHAR (255) NOT NULL ");
 $result = dbquery("ALTER TABLE ".DB_GR_RADIOSTATUS." ADD rs_service VARCHAR (255) NOT NULL ");
 $result = dbquery("ALTER TABLE ".DB_GR_RADIOSTATUS." ADD rs_box VARCHAR (255) NOT NULL ");*/
...
function rs_make_cache() {
	...
	$result = dbquery("SELECT rs_id, rs_ip, rs_port, rs_cache, rs_usertyp FROM ".DB_GR_RADIOSTATUS." WHERE rs_status='1'");
	if (dbrows($result)) {
		while ($data = dbarray($result)) {
		...
		if (isset($_GET['delete'])) {
			...
			if (isset($_GET['edit'])) {
				...
				if (dbrows($result)) {
					...
					$https = $data['rs_https'];
                    $rs_https_port = ' value="'.$data['rs_https_port'].'"';
                    $ps = ' value="'.$data['rs_ps'].'"';
					$tele = nl2br(stripslashes($data['rs_tele']));
					$flash = ' value="'.$data['rs_flash'].'"';
					$rs_fm = ' value="'.$data['rs_fm'].'"';
                    $rs_radio=' value="'.$data['rs_radio'].'"';
                    $rs_service=' value="'.$data['rs_service'].'"';
                    $rs_box=' value="'.$data['rs_box'].'"';
			...
			$rs_https = (isset($_POST['https']) && isnum($_POST['https']) ? $_POST['https'] : '0');
                $rs_https_port = (isset($_POST['rs_https_port']) ? stripinput($_POST['rs_https_port']) : (isset($_GET['edit']) ? $data['rs_https_port'] : ''));
				...
				$rs_fm = (isset($_POST['rs_fm']) ? stripinput($_POST['rs_fm']) : '');
                $rs_radio = (isset($_POST['rs_radio']) ? stripinput($_POST['rs_radio']) : (isset($_GET['edit']) ? $data['rs_radio'] : ''));
                $rs_service= (isset($_POST['rs_service']) ? stripinput($_POST['rs_service']) : (isset($_GET['edit']) ? $data['rs_service'] : ''));
                $rs_box= (isset($_POST['rs_box']) ? stripinput($_POST['rs_box']) : (isset($_GET['edit']) ? $data['rs_box'] : ''));
				...
				if ($rs_ip != '' && $rs_port != '' && $rs_pw != '') {
					if (isset($_GET['edit'])) {
						...
						$result_in = dbquery("INSERT INTO ".DB_GR_RADIOSTATUS." SET
                        rs_server_typ='".$rs_server_typ."',
                        rs_server_id='".$rs_server_id."',
                        rs_name='".$rs_name."',
                        rs_ip='".$rs_ip."',
			...
			<tr>
				<td class="tbl2" colspan="2"><strong>'.$locale['grrsa_37'].'</strong></td>
			</tr>
			<tr>
			...
			<tr>
                ...
                <option value="2"'.($server_typ == 2 ? ' selected="selected"' : '').'>LautFM [Beta!!!]</option>
                </select></td>
			</tr>
			<tr>
			...
			<tr>
				<td class="tbl1">'.$locale['grrsa_39'].' oder LautFM Station:</td>
				<td class="tbl1"><input type="text" name="ip" class="textbox" style="width:200px;"'.$ip.' /> <span style="color: red;">*</span></td>
			</tr>
			<tr>
				<td class="tbl1">'.$locale['grrsa_40'].'( LautFM Port: 5555 )</td>
				<td class="tbl1"><input type="text" name="port" class="textbox" style="width:200px;"'.$port.' /> <span style="color: red;">*</span></td>
			</tr>
		<tr>
				<td class="tbl1">HTTPS</td>
				<td class="tbl1"><select name="https" class="textbox" style="width:208px;">
                <option value="0"'.($https == 0 ? ' selected="selected"' : '').'>Nein</option>
                <option value="1"'.($https == 1 ? ' selected="selected"' : '').'>JA</option>

                </select></td>
			</tr>
            <tr>
				<td class="tbl1">HTTPS Port:</td>
				<td class="tbl1"><input type="text" name="rs_https_port" class="textbox" style="width:200px;"'.$rs_https_port.' /></td>
			</tr>
			...
			<tr>
				<td class="tbl2" colspan="2"><strong>'.$locale['grrsa_43'].'</strong></td>
			</tr>
			<tr>
				<td class="tbl1">'.$locale['grrsa_44'].'</td>
				<td class="tbl1"><select name="status_gb" class="textbox" style="width:208px;"><option value="1"'.($status_gb == 1 ? ' selected="selected"' : '').'>'.$locale['grrsa_58'].'</option><option value="0"'.($status_gb == 0 ? ' selected="selected"' : '').'>'.$locale['grrsa_59'].'</option></select></td>
			</tr>
			<tr>
				<td class="tbl1">'.$locale['grrsa_90'].'</td>
				<td class="tbl1"><select name="gb_popup" class="textbox" style="width:208px;"><option value="1"'.($gb_popup == 1 ? ' selected="selected"' : '').'>'.$locale['grrsa_58'].'</option><option value="0"'.($gb_popup == 0 ? ' selected="selected"' : '').'>'.$locale['grrsa_59'].'</option></select></td>
			</tr>
			<tr>
			...
			<tr>
				<td class="tbl2" colspan="2"><strong>'.$locale['grrsa_46'].'</strong></td>
			</tr>
			<tr>
			...
			<tr>
				<td class="tbl1" valign="top">Laut.Fm Stationsname (https://www.laut.fm/):</td>
				<td class="tbl1"><input type="text" name="rs_fm" class="textbox" style="width:200px;"'.$rs_fm.' /></td>
			</tr>
            <tr>
				<td class="tbl1" valign="top">Radio.de Stationsname (https://www.radio.de/s/):</td>
				<td class="tbl1"><input type="text" name="rs_radio" class="textbox" style="width:200px;"'.$rs_radio.' /></td>
			</tr>
            <tr>
				<td class="tbl1" valign="top">Radiodienste.de Stationsname (https://radiodienste.de/):</td>
				<td class="tbl1"><input type="text" name="rs_service" class="textbox" style="width:200px;"'.$rs_service.' /></td>
			</tr>
           <tr>
				<td class="tbl1" valign="top">Radiobox.de Stationsname (https://onlineradiobox.com/de/):</td>
				<td class="tbl1"><input type="text" name="rs_box" class="textbox" style="width:200px;"'.$rs_box.' /></td>
			</tr>

            <tr>
				<td class="tbl2" colspan="2"><strong>'.$locale['grrsa_50'].'</strong></td>
			</tr>
			<tr>
                ...
                <option value="6"'.($usertyp == 6 ? ' selected="selected"' : '').'>'.$locale['grrsa_64a'].'</option>
                </select>
                <a target="_blank" href="'.RADIOSTATUS.'dj-erkennung.php'.$aidlink.'">DJ-Erkennung SC2</a> </td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="tbl1"><select id="panel" name="panel" class="textbox" style="width:208px;"><option value="1"'.($panel == 1 ? ' selected="selected"' : '').'>'.$locale['grrsa_66'].'</option><option value="2"'.($panel == 2 ? ' selected="selected"' : '').'>'.$locale['grrsa_67'].'</option><option value="3"'.($panel == 3 ? ' selected="selected"' : '').'>'.$locale['grrsa_68'].'</option><option value="4"'.($panel == 4 ? ' selected="selected"' : '').'>'.$locale['grrsa_69'].'</option><option value="5"'.($panel == 5 ? ' selected="selected"' : '').'>'.$locale['grrsa_78'].'</option><option value="6"'.($panel == 6 ? ' selected="selected"' : '').'>'.$locale['grrsg_106'].'</option></select></td>
			</tr>
			<tr>
			...
			redirect(FUSION_SELF.$aidlink);
              ...
              if ($settings["site_protocol"] == "https") {
                                        if (substr(getcurrenturl(), 0, strlen(getcurrenturl()) - 5) . "/" != $settings["siteurl"]) {
                                            echo "<div class=\"admin-message\">" . $locale["grrsg_107"] . "</div>";
                                        }
                                    } else {
			...
			if (dbrows($result)) {
				...
				echo $locale['grrsa_15'];
                         } elseif ($data['rs_usertyp'] == 6) {
                          echo $locale['grrsa_64a'];
                        } else {
							echo $locale['grrsa_11'];
						}
						echo '</td>
			...
			/*	$date_opts = "<option value=''>".$locale['grrsg_95']."</option>\n";
                ...
                $date_opts = "<option value=''>".$locale['grrsg_95']."</option>\n";
				$date_opts .= "<option value='%m/%d/%Y'>".format_date("%m/%d/%Y", $timestamp)."</option>\n";
				$date_opts .= "<option value='%d/%m/%Y'>".format_date("%d/%m/%Y", $timestamp)."</option>\n";
				$date_opts .= "<option value='%d-%m-%Y'>".format_date("%d-%m-%Y", $timestamp)."</option>\n";
				$date_opts .= "<option value='%d.%m.%Y'>".format_date("%d.%m.%Y", $timestamp)."</option>\n";
				$date_opts .= "<option value='%m/%d/%Y %H:%M'>".format_date("%m/%d/%Y %H:%M", $timestamp)."</option>\n";
				$date_opts .= "<option value='%d/%m/%Y %H:%M'>".format_date("%d/%m/%Y %H:%M", $timestamp)."</option>\n";
				$date_opts .= "<option value='%d-%m-%Y %H:%M'>".format_date("%d-%m-%Y %H:%M",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d.%m.%Y %H:%M'>".format_date("%d.%m.%Y %H:%M",$timestamp)."</option>\n";
				$date_opts .= "<option value='%m/%d/%Y %H:%M:%S'>".format_date("%m/%d/%Y %H:%M:%S",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d/%m/%Y %H:%M:%S'>".format_date("%d/%m/%Y %H:%M:%S",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d-%m-%Y %H:%M:%S'>".format_date("%d-%m-%Y %H:%M:%S",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d.%m.%Y %H:%M:%S'>".format_date("%d.%m.%Y %H:%M:%S",$timestamp)."</option>\n";
				$date_opts .= "<option value='%B %d %Y'>".format_date("%B %d %Y",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d. %B %Y'>".format_date("%d. %B %Y",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d %B %Y'>".format_date("%d %B %Y",$timestamp)."</option>\n";
				$date_opts .= "<option value='%B %d %Y %H:%M'>".format_date("%B %d %Y %H:%M",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d. %B %Y %H:%M'>".format_date("%d. %B %Y %H:%M",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d %B %Y %H:%M'>".format_date("%d %B %Y %H:%M",$timestamp)."</option>\n";
				$date_opts .= "<option value='%B %d %Y %H:%M:%S'>".format_date("%B %d %Y %H:%M:%S",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d. %B %Y %H:%M:%S'>".format_date("%d. %B %Y %H:%M:%S",$timestamp)."</option>\n";
				$date_opts .= "<option value='%d %B %Y %H:%M:%S'>".format_date("%d %B %Y %H:%M:%S",$timestamp)."</option>\n";
				...
				<tr>
					...
					<tr>
					<td class="tbl1">Gru&szlig; bzw. und Wunschbox Captcha:</td>
				<td class="tbl1">
...
#echo '<div align="center">'.$locale['grrsa_76'].'<a href="http://www.granade.eu/scripte/radiostatus.html" target="_blank"><img src="http://www.granade.eu/scripte/radiostatus_26.jpg" border="0" alt="Version-Check" /></a><br />'.$locale['grrsa_79'].'</div>';
   include INFUSIONS.'gr_radiostatus_panel/version_check.php';

/* Das entfernen des Copyright ist nicht erlaubt und ist Strafbar! */
echo '<div class="small2" align="center"><hr class="side-hr" /><a href="http://www.granade.eu/scripte/radiostatus.html" target="_blank">Radiostatus &copy;</a> Mod BY <a href="https://fusion-club24.de" target="_blank">21Matze &copy;</a></div>';
closetable();
